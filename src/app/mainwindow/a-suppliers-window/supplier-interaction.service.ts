...
export class SupplierInteractionService {
  ...
  getSupplier() {
    this.http.get<{message: string, suppliers: any}>('http://localhost:3002/api/supplier')
  ...
  getSuppiers(id: string){
    ...
    ('http://localhost:3002/api/supplier/' + id);
  }
  ...
  addSupplier( supplierID: string, name: string, email: string, contact: string, drugsAvailable: string) {
    ...
    this.http.post<{message: string, supplierId: string}>('http://localhost:3002/api/supplier',supplier)
  ...
  updateSupplier(id: string , supplierID: string , name: string, email: string, contact: string, drugsAvailable: string){
    ...
    this.http
             .put('http://localhost:3002/api/supplier/' + id , supplier)
             .subscribe(response => {
  ...
  deleteSupplier(supplierId: string) {
    this.http.delete('http://localhost:3002/api/supplier/' + supplierId)
      .subscribe(() => {
